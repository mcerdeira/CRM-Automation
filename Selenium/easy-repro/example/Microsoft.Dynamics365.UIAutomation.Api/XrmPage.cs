...
namespace Microsoft.Dynamics365.UIAutomation.Api
{
    ...
    public class XrmPage : BrowserPage
    {
        ...
        public void SetValueJSLookUp(string fieldname, string guid, string name, string entity, bool fireOnChange = false)
        {
            string change = "";
            if (fireOnChange)
            {
                change = string.Format("Xrm.Page.getAttribute('{0}').fireOnChange();", fieldname);
            }

            string command = string.Format(@"var lookupValue = new Array();
            lookupValue[0] = new Object();
            lookupValue[0].id = '{1}'
            lookupValue[0].name = '{2}'
            lookupValue[0].entityType = '{3}';
            Xrm.Page.getAttribute('{0}').setValue(lookupValue);
            {4}
            ", fieldname,guid, name, entity, change);
            this.Browser.Driver.ExecuteScript(command);
        }

        public void SetValueJS(string field, string value, bool fireOnChange = false)
        {
            string change = "";
            if (fireOnChange) 
            {
                change = string.Format("Xrm.Page.getAttribute('{0}').fireOnChange();", field);
            }

            string command = string.Format("Xrm.Page.getAttribute('{0}').setValue({1});{2}", field, value, change);
